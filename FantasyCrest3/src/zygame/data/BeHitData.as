   ...
   public class BeHitData
   {
      ...
      public function getHurt(attribute:RoleAttributeData) : int
      {
         ...
         if(armorScale > 0)
         {
            ...
            {
               if(role.attribute.power <= attribute.armorDefense)
               {
                  wvalue *= role.attribute.power / attribute.armorDefense;
               }
               else
               {
                  wvalue *= 1 - (1 - attribute.armorDefense / role.attribute.power) * 0.5;
               }
            }
            hurt += wvalue;
         }
         if(magicScale > 0)
         {
            ...
            if(role.attribute.magic <= attribute.magicDefense)
            {
               mvalue *= role.attribute.magic / attribute.magicDefense;
            }
            else
            {
               mvalue *= 1 - (1 - attribute.magicDefense / role.attribute.magic) * 0.5;
            }
            hurt += mvalue;
         }
         if(hurt <= 0)
